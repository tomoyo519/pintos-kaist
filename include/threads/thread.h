...
struct thread
{
	...
	int init_priority;		   /* 초기 priority 를 가지고 있어야 함*/
	struct lock *wait_on_lock; /* 이 락을 얻기 위해 대기하고있는 쓰레드 1개.*/
	struct list donators;	   /* 나에게 priority를 기부해준 쓰레드 list*/
	struct list_elem d_elem;   /* donators를 연결해주는 elem */
#ifdef USERPROG
